  ...
  (list
   ...
   (list
    (concat "/\\*\\(?:\\*\\|!\\)+[ \t]*"
	    ...
	    "\\(?:\\(\\.\\)\\|"
	         "\\(?:[ \t]*\n[ \t]*\\)-#?\\|"
		 "\\(?:[ \t]*\n[ \t]*\\)?[\\@]\\(?:overload\\|name\\|li\\|ingroup\\|internal\\|par\\|param\\|return\\|warning\\|note\\|todo\\|bug\\|verbatim\\|tparam\\|\\(?:main\\)?page\\)\\|"
		 "\\(?:[ \t]*\n[ \t]*\\)<!--+ *!+ *--+>\\|"
		 "[ \t]*\\*+/\\)")
   ...
   (list
    "\\([@\\\\]b\\)\\s-+\\([^ \t\n]+\\)"
    '(1 font-lock-semi-unimportant prepend)
    '(2 (quote bold) prepend))
   ;; code
   (list
    "\\([@\\\\][cp]\\)\\s-+\\([^ \t\n]+\\)"
   ...
   (list
    (concat "\\([@\\\\]\\(addtogroup\\|defgroup\\|weakgroup"
	    "\\|page\\|anchor\\|section\\|subsection"
	    "\\)\\)\\s-+\\(\\sw+\\)\\(.*$\\)")
    '(1 font-lock-keyword-face prepend)
    '(3 font-lock-string-face prepend))))
...
(defun doxymacs-font-lock ()
  ...
  (if (functionp 'font-lock-add-keywords)
      ...
      (font-lock-add-keywords nil doxymacs-doxygen-keywords t)
    ;; Use old-school way
