...
Search the directories in `compilation-search-path'. A nil in
`compilation-search-path' means to
- try the \"current\" directory, which is passed in
  DIRECTORY. If DIRECTORY is relative, it is combined with
  `default-directory'.
...
attempts to find a file whose name is produced by (format FMT FILENAME)."
  ...
  (let* ((dirs compilation-search-path)
	 (spec-dirs (append (when directory (list (expand-file-name directory))) (list default-directory)))
    ...
    (while (and dirs (null buffer))
      (when (null (car dirs))
	(setq dirs (append spec-dirs (cdr dirs))))
