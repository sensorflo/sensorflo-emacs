...
(defconst camelCase-keybindings-list
  (cond ((memq 'xemacs features) ;; xemacs uses different key syntax
	 '(
	   ("\M-l"              camelCase-forward-word)
	   ("\M-j"              camelCase-backward-word)
	   ("\M-d"              camelCase-forward-kill-word)
	   ("\M-DEL"            camelCase-backward-kill-word)
	   ("\M-t"              camelCase-transpose-words)
	   ;("\M-c"              camelCase-capitalize-word) ; normaly capitalize-region-or-word
	   ;("\M-u"              camelCase-upcase-word) ; normaly upcase-region-or-word
	   ("\C-m"              camelCase-mark-word)
...
(defvar camelCase-mode-map (make-sparse-keymap)
...
(defun camelCase-forward-word (count)
  "move point forward COUNT camelCase words"
  (interactive "p") 
