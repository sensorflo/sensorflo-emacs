...
(defconst qml--property-re
  (concat "\\(?:\\(readonly\\)[ \t]+\\)?\\(property\\)[ \t]+\\(" (qml--list-to-string qml--property) "\\)[ \t]+\\([a-zA-Z_][a-zA-Z_0-9]*\\)"))
...
(defconst qml--font-lock-keywords-1
  `( ;; Keywords
    (,qml--class-re (0 'font-lock-type-face))
    (,js--keyword-re (0 'font-lock-keyword-face))
    ;; Types
    (,qml--type-re (1 'font-lock-keyword-face))
...
(defconst qml--font-lock-keywords-2
  `(;; This goes before keywords-1 so it gets used preferentially
    ...
    ;; commented out.
    (,qml--property-re (1 'font-lock-keyword-face nil t)
...
(defun qml--gen-font-lock-keywords (string-list face)
  (list
   (list (js--regexp-opt-symbol string-list) 0 face)))
